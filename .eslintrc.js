module.exports = {
   ...
   问题类型查看
    ...
    'rules': {
        ...
        // 'vue/require-v-for-key': 'error', // 不要把 v-if 和 v-for 用在同一个元素上——因为v-for 比 v-if 具有更高的优先级
        // 'vue/v-bind-style': ['error', 'shorthand'], // 指令缩写 (用 : 表示 v-bind: 和用 @ 表示 v-on:) 应该要么都用要么都不用
        // 'vue/v-on-style': ['error', 'shorthand'],// 指令缩写 (用 : 表示 v-bind: 和用 @ 表示 v-on:) 应该要么都用要么都不用
        'indent': ['error', 4],// 缩进风格
